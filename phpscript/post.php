...
for ($rf = 0; $rf < 1; $rf++) {
	...
	if ($_G['SET']['POST' . strtoupper($type) . 'CHECK'] && !InArray($_G['USER']['QUANXIAN'], 'nopost' . $type . 'check') && !$postarray['del']) {
		$postarray['del'] = 2;
	}
...
if (Cnum($r)) {
	...
	if ($postarray['del'] < 2) {
		...
		if ($_G['GET']['RETURN'] == 'json') {
			exit(json_encode(array('state' => 'ok', 'msg' => '发表成功请等待审核通过', 'rid' => $r)));
		} else {
			$_G['HTMLCODE']['TIP'] = '发表成功请等待审核通过';
