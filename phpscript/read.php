...
if ($readdata) {
	...
	if (!$chkr) {
		...
		//seo重塑
		$_G['SET']['WEBTITLE'] = $_G['SET']['WEBKEYWORDS'] = strip_tags($readdata['title']);
		$_G['SET']['WEBDESCRIPTION'] = mb_substr(str_replace('	', '', str_replace(' ', '', str_replace("\r", '', str_replace("\n", '', htmlspecialchars(strip_tags($readdata['content']), ENT_QUOTES))))), 0, 200, 'utf-8');
	...
	$readdata = $_G['TABLE']['READ'] -> getData(array('id' => $id));
	if ($readdata) {
		if ($readdata['del'] = 1) {
			//回收站
			$_G['HTMLCODE']['TIP'] = '该主题已被移至回收站！';
		} else {
			//审核或其他
			$_G['HTMLCODE']['TIPJS'] = 'location.href="index.php?c=list"';
			$_G['HTMLCODE']['TIP'] = '该主题正在审核，请等待管理处理';
		}
	} else {
		//不存在或以删除的主题
		$_G['HTMLCODE']['TIP'] = '不存在的主题！';
	}
	$_G['HTMLCODE']['OUTPUT'] .= template('tip', TRUE);
}
