...
$(function() {
	//获取导航数据
	$.getJSON('index.php?c=app&a=superadmin&s=nav', function(data) {
		nav_data = data;
		var h_n = '';
		//关闭启动界面
		$('.start-div').remove();
		if($(window).width() > 1000) {
			...
			//布局整理
			$('#app-superadmin-mainshow-sm').css('height', $('#app-superadmin-main-sm').height() - $('#app-superadmin-main-sm>div:eq(0)').outerHeight() - $('#app-superadmin-main-sm>div:eq(2)').outerHeight() + 'px');
			$('#app-superadmin-main-sm>div:eq(1)').css('height', $('#app-superadmin-mainshow-sm').height());
			//写入导航数据
			h_n = '';
			for(var i = 0; i < data.length; i++) {
			...
			$('#app-superadmin-main-sm>div:eq(0)>div:eq(0)>select').html(h_n).change(function() {
				...
				$('#app-superadmin-main-sm>div:eq(0)>div:eq(1)>select').html(h_n).change(function() {
					...
					//显示正在加载tip
					pktip('<span class="fa fa-fw fa-spin fa-spinner"></span>正在加载...');
					...
					if(!nav_data[PIndex].data[Index].target) {
						$('#app-superadmin-mainshow-sm iframe').attr('src', t);
					} else {
