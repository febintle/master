...
if (!$chkr) {
	...
	//判断该用户下载是否扣费
	if ($_SESSION['APP_PUYUETIANEDITOR_DOWNLOADSESSION_' . $_G['USER']['ID'] . '_' . $downloaddata['id']] != 'ok' && (((strpos($downloaddata['downloadeduids'], "_{$_G['USER']['ID']}_") === FALSE && $_G['SET']['DOWNLOADEDRECORD']) || !$_G['SET']['DOWNLOADEDRECORD']) || !$_G['SET']['DOWNLOADEDRECORD'])) {
		//扣费
		$_G['TEMP']['CHKJFTD'] = 1;
	} else {
		//不扣费
