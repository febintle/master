...
foreach ($_FILES['file']['error'] as $key => $value) {
	if ($_FILES['file']['error'][$key] > 0) {
		...
		uploadexit("出错，{$_FILES['file']['error']}:{$errortip}");
	}
	...
	//&& $suffix != 'gif'
	if ($_G['GET']['T'] == 'image' && count($compress_wh) == 2) {
		$image = FALSE;
	...
	//图片加水印
	if ($_G['GET']['T'] == 'image' && $_G['SET']['APP_PUYUETIANEDITOR_WATERMARK_TEXT']) {
		$image = imagecreatefromstring(file_get_contents($useruploadfilespath . $filename));
	...
	if ($_G['GET']['T'] != 'file') {
		...
		if ($_G['GET']['T'] == 'image') {
			$jscode .= 'parent.document.getElementById("PytImagesBox").innerHTML+="<div class=\"pk-w-sm-3 pk-margin-bottom-10\"><img class=\"pk-active\" src=\"' . $url . '\" onclick=\"$(this).toggleClass(\'pk-active\')\"></div>";';
		} else {
...
<script>
' . $jscode . ';
location.href="app/puyuetianeditor/template/upload.html?t=' . $_G['GET']['T'] . '&rnd=' . $_G['RND'] . '";
</script>
');
