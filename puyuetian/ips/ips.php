...
foreach ($ips as $ip) {
	...
	if ($ban) {
		...
		$_G['HTMLCODE']['MAIN'] .= template($_G['SYSTEM']['PATH'] . '/template/default/null.html', TRUE);
		$_G['HTMLCODE']['MAIN'] .= template($_G['SYSTEM']['PATH'] . '/template/default/siteclosed.html', TRUE);
		$_G['HTMLCODE']['MAIN'] .= template($_G['SYSTEM']['PATH'] . '/template/default/null.html', TRUE);
		template($_G['SYSTEM']['PATH'] . '/template/default/main.html');
		exit();
	}
}
